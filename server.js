...
function createComment(url, request) {
  // It returns the value of the second operand if the first is truthy,
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    ...
    // Takes the value of the first expression if it is not null, undefined, false.
    // Otherwise, it takes the value of the second expression.
...
function upvoteComment(url, request) {
  ...
  if (savedComment && database.users[username]) {
    // Refresh the Upvoted By attribute in the comment section.
